...
from cccv.arch.arch_util import ResidualBlockNoBN, pixel_unshuffle
from cccv.type import ArchType


@ARCH_REGISTRY.register(name=ArchType.MSRSWVSR)
class MSRSWVSR(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.pad(x, (0, pad_w - w, 0, pad_h - h, 0, 0), "replicate")
        # initialize previous sr frame and previous hidden state as zero tensor
        out = x.new_zeros(b, c, pad_h * self.netscale, pad_w * self.netscale)
        state = x.new_zeros(b, self.num_feat, pad_h, pad_w)
        out_l = []
        for i in range(n):
            if i == 0:
        ...
        x = torch.stack(out_l, dim=1)
        return x[:, :, :, : h * self.netscale, : w * self.netscale]
